...
const SymbolListOrganizer = ({ userId }) => {
  ...
  const fetchLists = async () => {
    ...
    if (error) {
      ...
      const listsWithValidSymbols = data.map(list => {
        // Parse symbols array from database
        let parsedSymbols = [];
        if (list.symbols) {
          parsedSymbols = list.symbols.map((symbol, index) => {
            // If symbol is a string, create an object
            ...
            // If symbol is already an object with content
            if (typeof symbol === 'object' && symbol.content) {
              return {
                id: symbol.id || `${list.id}-${index}`,
                content: symbol.content
              };
            }
            // If symbol is the raw content
            return {
              id: `${list.id}-${index}`,
              content: symbol
            };
  ...
  const addSymbol = async (listId) => {
    if (newSymbol.trim() !== '') {
      ...
      // When saving to database, only save the content
