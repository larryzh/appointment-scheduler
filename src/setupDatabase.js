...
export async function setupDatabase() {
  ...
  try {
    ...
    const { error: sectionsError } = await supabase.rpc('create_sections_table');
    if (sectionsError) {
...
-- Function to create the sections table
...
begin
  ...
  create policy "Users can insert their own sections" on sections for insert with check (true);
  create policy "Users can update their own sections" on sections for update using (true);
  create policy "Users can delete their own sections" on sections for delete using (true);
end;
$$;
...
begin
  create table if not exists public.groups (
    id bigint primary key generated by default as identity,
    user_id bigint references public.users not null,
    section_id bigint references public.sections,
    name text not null,
    position integer not null default 0,
    created_at timestamp with time zone default timezone('utc'::text, now()) not null
  );
  ...
  create index if not exists idx_groups_section_id on public.groups(section_id);
...
select create_sections_table();
select create_groups_table();
select create_lists_table();

-- Add section_id to groups if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    ...
    ALTER TABLE public.groups ADD COLUMN section_id bigint references public.sections;
    CREATE INDEX idx_groups_section_id ON public.groups(section_id);
  END IF;
END $$;
*/
